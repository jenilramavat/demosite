...
;(function($, window, undefined)
{
    ...
    $(document).ready(function()
    {
        ...
        neonLogin.$container = $("#form_portal_login");
        neonLogin.$container.validate({
            rules: {
                username: {
                    required: true
            ...
            submitHandler: function(ev)
            {
                ...
                setTimeout(function()
                {
                    ...
                    //alert( JSON.stringify(neonLogin.$container.serializeArray()));
                    var postData = $("#form_portal_login").serializeArray();

                    // Send data to the server
                    $.ajax({
                        url: baseurl + '/portal/dologin',
                        method: 'POST',
                        dataType: 'json',
                        data: postData,/*{
                         email: $("input#email").val(),
     ...
     ///portal
        $("#user_language_p ul li").click(function(){
            window.location = baseurl + "/portal/login?lang="+$(this).attr("lang-key");
            return false;
        });
        $("#user_language_p.language-selector .dropdown-toggle img").attr("src",$("#user_language_p ul li.active img").attr("src"));
        $("#user_language_p.language-selector .dropdown-toggle span").html($("#user_language_p ul li.active span").html());
        if(typeof customer_alignment!="undefined" && customer_alignment=="right"){
            $('.pull-right, .pull-left').addClass('flip');
        }
        if($("user_language_p ul li.active").length){
            var page_url = baseurl + "/portal/login?lang="+$("#user_language_p ul li.active").attr("lang-key");
            window.history.pushState({path:page_url},'',page_url);
        }
        $("#user_language_p ul li").click(function(){
            window.location = baseurl + "/portal/login?lang="+$(this).attr("lang-key");
