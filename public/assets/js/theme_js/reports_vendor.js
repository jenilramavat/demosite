...
function loadTable(table_id,pageSize,$searchFilter){
    ...
    "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        if ($searchFilter.margin_show == 1 && parseFloat(aData[6].replace(',','')) < 0) {
...
function loadTable1(table_id,pageSize,$searchFilter){
    ...
    var TotalMargin = 0;
        ...
        "bServerSide": true,
        "bAutoWidth": false,
        "sAjaxSource": baseurl + "/vendor_analysis/ajax_datagrid1/type",
        "fnServerParams": function (aoData) {
            aoData.push(
        ...
        "aButtons": [
            {
                "sExtends": "download",
                "sButtonText": BUTTON_EXPORT_EXCEL_CAPTION,
                "sUrl": baseurl + "/vendor_analysis/ajax_datagrid1/xlsx", //baseurl + "/generate_xlsx.php",
                sButtonClass: "save-collection"
            },
            {
                "sExtends": "download",
                "sButtonText": BUTTON_EXPORT_CSV_CAPTION,
                "sUrl": baseurl + "/vendor_analysis/ajax_datagrid1/csv", //baseurl + "/generate_csv.php",
                sButtonClass: "save-collection"
            }
        ]
    },
    "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        if ($searchFilter.margin_show == 1 && parseFloat(aData[10].replace(',','')) < 0) {
            $('td', nRow).css('background-color', 'Red').css('color', 'white');
        }
    },
    "fnDrawCallback": function () {
    ...
    "fnFooterCallback": function ( row, data, start, end, display ) {
        if (end > 0) {
            ...
            $($(row).children().get(0)).html('<strong>'+TABLE_TOTAL+'</strong>');  
            $($(row).children().get(4)).html('<strong>'+TotalDuration+'</strong>');
            $($(row).children().get(5)).html('<strong>'+TotalCall+'</strong>');
            $($(row).children().get(6)).html('<strong>'+TotalFailCall+'</strong>');
            $($(row).children().get(7)).html('<strong>'+SumOfCalls+'</strong>');
            if(TotalCost) {
                $($(row).children().get(12)).html('<strong>' + TotalCost + '</strong>');
            }
            if(TotalMargin) {
                $($(row).children().get(11)).html('<strong>' + TotalMargin + '</strong>');
            }
        }else{
            $(table_id).find('tfoot').find('tr').html('');
        }
